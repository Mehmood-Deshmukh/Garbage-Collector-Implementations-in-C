...
void test_init(){
    HashMap map;
    hashmap_init(&map);
    assert_equal(HASHMAP_SIZE, map.size, "Size should be initialized");
    for(int i = 0; i < HASHMAP_SIZE; i++){
        assert_equal((uintptr_t )NULL, (uintptr_t )map.buckets[i], "Buckets should be initialized");
    }
...
void test_insert_and_lookup(){
    ...
    uintptr_t value = 0xfff000000001;
...
void test_multiple_inserts(){
    ...
    uintptr_t base_value = 0xfff000000001;
    for(int i = 0; i < n; i++){
        uintptr_t key = base_address + i;
        uintptr_t value = base_value + i;
        hashmap_insert(&map, key, value);
    }
    for(int i = 0; i < n; i++){
        uintptr_t key = base_address + i;
...
void test_update(){
    ...
    uintptr_t key = 0x7ff000000000;
    uintptr_t value = 0xfff000000001;
    uintptr_t updated_value = 0xfff000000002;
    hashmap_insert(&map, key, value);
...
void test_iterator(){
    ...
    while(hashmap_iterator_has_next(iter)){
        ...
        uintptr_t expected_value = base_value + key - base_address;
        assert_equal(expected_value, value, "Iterator should return correct value");
        count++;
    }
...
void test_collision_handling(){
    ...
    hashmap_init(&map);
    uintptr_t key1 = base;
    uintptr_t value1 = 0xfff000000001;
    uintptr_t key2 = base + i;
    uintptr_t value2 = 0xfff000000002;
...
void test_stress(){
    ...
    hashmap_init(&map);
    int n = 100000;
    uintptr_t base_address = 0x000000000000;
    uintptr_t base_value = 0xf00000000001;
    for(int i = 0; i < n; i++){
        uintptr_t key = base_address + i;
        uintptr_t value = base_value + i;
        hashmap_insert(&map, key, value);
